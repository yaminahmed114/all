...
std::vector<float> Bresenham(float x0, float y0, float x1, float y1)
{
    ...
    points.push_back(x0);
    points.push_back(y0);
    points.push_back(0.0f); // Z value (always 0 in 2D)

    while (x0 < x1) 
    {
        if (d < 0) 
        {
            d = d + dS; // Move in the x direction
        } 
        
        else 
        {
            d = d + dT; // Move in the x and y direction
            y0++; // Increment y
        }
        ...
        // Store the point in NDC
        points.push_back(x0);  // Convert back to NDC
        points.push_back(y0);  // Convert back to NDC
        points.push_back(0.0f);         // Z value (always 0 in 2D)
    }
